...
export class AuthService {
    ...
    const [accessToken, refreshToken] = await Promise.all([
      this.jwtService.signAsync(
        { sub: userId, email },
        {
          ...
          expiresIn: this.configService.get<string>('ACCESS_TOKEN_TTL') || '15m' // Durée de validité du token d'accès,
        },
      ),
      this.jwtService.signAsync(
        { sub: userId, email },
        {
          ...
          expiresIn: this.configService.get<string>('REFRESH_TOKEN_TTL') || '1y', // Durée de validité du token de rafraîchissement,
        },
      ),
    ]);
  ...
  async refreshTokens(userId: number, refreshToken: string) {
    const user = await this.userService.findOneWithCurrentHashedRefreshToken(userId);
